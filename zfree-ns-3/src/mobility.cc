...
NodeStatistics::RxCallback (std::string path, Ptr<const Packet> packet, const Address &from)
{
    ...
    //std::cout << "bytes total: " << m_bytesTotal << "\n";
}
...
NodeStatistics::AdvancePosition (Ptr<Node> node, int stepsSize, int stepsTime)
{
    ...
    totaltime += stepsTime;
    std::cout << "mbs: " << mbs << " at time: " << totaltime <<'\n';
    pos.x += stepsSize;
...
//   LogComponentEnable ("LteUeNetDevice", logLevel);
  ...
  double yForUe = 530.0;   // m
  double speed = 200;       // m/s
  double simTime = (double)(numberOfEnbs + 1) * distance / speed; // 1500 m / 20 m/s = 75 secs
  double enbTxPowerDbm = 46.0;
    double interPacketInterval = 10.0;
    double stepsSize = 1.0;
    double stepsTime = 2.0;
  ...
  ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (0, yForUe, 200));
...
//            serverApp = udpServer.Install (ueNodes.Get (0));
          ...
          PacketSinkHelper dlPacketSinkHelper ("ns3::UdpSocketFactory",
            ...
            UdpClientHelper dlClientHelper (ueIpIfaces.GetAddress (u), dlPort);//downlink
            dlClientHelper.SetAttribute ("Interval", TimeValue (MilliSeconds(interPacketInterval)));
            dlClientHelper.SetAttribute ("MaxPackets", UintegerValue(1000000));
            
            UdpClientHelper ulClientHelper (remoteHostAddr, ulPort);//uplink
  ...
  pdcpStats->SetAttribute ("EpochDuration", TimeValue (Seconds (1.0)));
    ...
    //Config::Connect ("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/RemoteStationManager/$" + apManager + "/RateChange", MakeCallback (RateCallback));
...
//
//
    NodeStatistics atpCounter = NodeStatistics (ueLteDevs, enbLteDevs);
    Simulator::Schedule (Seconds (0.5 + stepsTime), &NodeStatistics::AdvancePosition, &atpCounter, ueNodes.Get (0), stepsSize, stepsTime);
...
MakeCallback (&NodeStatistics::RxCallback, &atpCounter));
  ...
  // config.ConfigureAttributes ();
    ...
    {
        Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow (i->first);
        NS_BUILD_DEBUG (std::cout << "Flow " << i->first << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n");
        NS_BUILD_DEBUG (std::cout << "  Tx Bytes:   " << i->second.txBytes << "\n");
        NS_BUILD_DEBUG (std::cout << "  Rx Bytes:   " << i->second.rxBytes << "\n");
        NS_BUILD_DEBUG (std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << " Mbps\n");
    }

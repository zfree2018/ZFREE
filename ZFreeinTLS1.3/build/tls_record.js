...
const encryptRecord = function (pv, index, table, r) {
	//    console.log(keys);
	switch (pv) {
		...
		case formats.PV.TLS_1p3:
			{
				...
				if (f && f.valid) return Correct({
					type: f.ct,
					version: formats.PV.TLS_1p3,
					fragment: f.plaintext
				});else return Incorrect(AD.decode_error, "decryptRecord: authenticated decryption failed");
			}
		default:
			{
...
const logRecord = function (label, msg, pv, kex, from_server) {
	...
	if (msg.type === formats.ContentType.change_cipher_spec) {
		...
		console.log(label + " Alert(" + formats.AD_lookup(msg.fragment) + ") [" + msg.version + "]");
	}
	if (msg.type === formats.ContentType.handshake) {
		let fm = formats.parseMessage(msg.fragment);
		bindError(fm, hsmsg => {
			let x = hsmsg.fst;
			if (x == null) throw new Error("expected hs_msg");else {
				let y = x;
				return bindError(formats.parseHandshakeMessage(x, pv, kex, from_server), m => {
					y.pl = m;
					console.log(label + " HS(" + JSON.stringify(tls_pp.printHandshakeMessage(y), null, '  ') + ") [" + msg.version + "]");
...
const outputRecord = function (cipherState, msg, from_server) {
	logRecord("Sending", msg, cipherState.params.pv, cipherState.params.kex, from_server);
	if (cipherState.write == true) {
		let index;
		if (cipherState.role === "client") index = cipherState.params.cr;else index = cipherState.params.sr;
...
const inputRecord = function (cipherState, msg, from_server) {
	let res = msg;
	if (cipherState.read == true) {
		...
		if (dec.result === 'Error') {
			throw new Error("Could not decrypt record, aborting - " + JSON.stringify(msg));
		} else if (dec.result === 'Correct') {
			res = dec.value;
		}
	};
	logRecord("Received", res, cipherState.params.pv, cipherState.params.kex, from_server);
	if (res.type === formats.ContentType.application_data) {
		cipherState.payload += util.hex2a(res.fragment);
	}
	return res;
};
const TLS_client_scenario = function (sn, p, cb) {
	const parsed = {};
	let path = '/';
	if (/^https/.test(sn)) {
...
const TLS_server_scenario = function (p, cb) {
	tls_server(p, handshake.TLS_server_callbacks, cb);
};

const tls_client = function (host, path, port, callbacks, close_cb) {
	...
	const sendHmac = function (msg) {
		console.log("sending hmac msg with handshake keys");
		msg = tls_crypto.hmac_sha256(cipherState.write_keys, msg)
		let res = formats.serializeRecord(msg);
		client.write(res, 'hex');
	};
	client.on("connect", () => {
		console.log('Connected');
		if (hs_state != 0) throw new Error("sending client hello when state != 0");
		if (callbacks.hs_send_client_hello != undefined) {
			let send = callbacks.hs_send_client_hello(cipherState);
			send.forEach(sendRecord);
		}
		if (callbacks.hs_send_client_finished0 != undefined && cipherState.params.pv === formats.PV.TLS_1p3) {
			let send = callbacks.hs_send_client_finished0(cipherState);
			send.forEach(sendRecord);
		}
		if (callbacks.app_data_send0 != undefined && cipherState.params.pv === formats.PV.TLS_1p3) {
			let send = callbacks.app_data_send0(cipherState);
			//send.forEach(sendRecord);
			send.forEach(sendHmac);
		}
		hs_state = 1;
	});
	...
	const deliver = function (msg) {
		...
		if (msg.type === formats.ContentType.handshake) {
			...
			while (more) {
				...
				mapError(formats.parseHandshakeMessage(pmsg, cipherState.params.pv, cipherState.params.kex, true), m => {
					let nm = pmsg;
					nm.pl = m;
					hs_msgs.push(nm);
					if (nm.ht === formats.HT.server_hello) {
						...
						if (callbacks.hs_recv_server_hello != undefined) {
							let send = callbacks.hs_recv_server_hello(hs_msgs, cipherState);
							send.forEach(sendRecord);
						}
						hs_msgs = [];
						hs_state = 2;
						...
						if (callbacks.hs_recv_server_hello_done != undefined) {
							...
							let send = callbacks.hs_recv_server_hello_done(hs_msgs, cipherState);
							send.forEach(sendRecord);
						}
						hs_msgs = [];
						hs_state = 3;
...
const tls_server = function (port, callbacks, close_cb) {
	net.createServer(function (server) {
		...
		const sendHmac = function (msg, cp) {
			console.log("sending hmac msg with handshake keys");
			//msg = tls_crypto.hmac_sha256(cipherState.write_keys, msg)
			let res = formats.serializeRecord(msg);
			server.write(outputRecord(cp, msg, true), 'hex');
		...
		function deliver(msg) {
			...
			if (msg.type === formats.ContentType.alert) {
				...
				if (cipherState.expect_early_data && msg.fragment == formats.AD.end_of_early_data && callbacks.end_of_early_data_recv != undefined) {
				...
				const ad = msg;
				if (cipherState.expect_early_data) {} else if (callbacks.app_data_recv != undefined) {
					let send = callbacks.app_data_recv(ad.fragment, cipherState);
					send.forEach(function (v) {
						sendHmac(v, cipherState);
						//sendRecord(v, cipherState);
					});
				};
					...
					switch (msg.result) {
						...
						case "Correct":
							if (msg.value.fst == null) {
								...
								let nm = msg.value.fst;

								hs_buf = msg.value.snd;
								mapError(formats.parseHandshakeMessage(nm, cipherState.params.pv, cipherState.params.kex, false), m => {
									nm.pl = m;
									hs_msgs.push(nm);
            ...
            throw new Error("Client Finished received when hs_state != 2");
										...
										if (callbacks.hs_recv_client_finished0 != undefined && cipherState.expect_early_data) {
											let send = callbacks.hs_recv_client_finished0(hs_msgs, cipherState);
											send.forEach(function (v) {
												sendRecord(v, cipherState);
											});
										} else if (callbacks.hs_recv_client_finished != undefined) {
											let send = callbacks.hs_recv_client_finished(hs_msgs, cipherState);
											send.forEach(function (v) {
												sendRecord(v, cipherState);
											});
										}

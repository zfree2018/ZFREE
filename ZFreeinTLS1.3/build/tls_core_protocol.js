...
const checkClientHelloLog = function (log, cr, gx) {
			return true;
};
const checkServerHelloLog = function (log, params) {
			return true;
};
const checkServerCertificateLog = function (log, params, serverId) {
			return true;
};
const checkServerCertificateVerifyLog = function (log, params, serverId) {
			return true;
};
const checkServerFinishedLog = function (log, params, serverId) {
			return true;
};
const checkClientCertificateLog = function (log, params, serverId, clientId) {
			return true;
};
const checkClientCertificateVerifyLog = function (log, params, serverId, clientId) {
			return true;
};

const checkClientKeyExchangeLog12 = function (log, params, serverId) {
			return true;
};
const checkClientCertificateLog12 = function (log, params, serverId, clientId) {
			return true;
};
const checkServerKeyExchangeParams12 = function (to_sign, gy) {
			return true;
};
const checkServerPreFinishedLog12 = function (log, params, serverId) {
			return true;
};
...
var deriveKeys_12 = function (ae, ms, ctx) {
			switch (ae) {
...
var encrypt_12 = function (keys, plain, ad) {
			switch (keys.ae) {
						case "AES_128_GCM_SHA256":
...
var decrypt_12 = function (keys, cipher, ad) {
			switch (keys.ae) {
						case "AES_128_GCM_SHA256":
...
var dh_keygen = function (gn) {
			switch (gn) {
						...
						default:
									throw new Error("only p256/ff2048 supported");
			}
};

var gen_dh_keys = function () {
...
var dh_compute_secret = function (gn, x, gy) {
			switch (gn) {
						case "p256":
									return ProScript.crypto.p256_ecdh(x, gy);
						case "ff2048":
									return ProScript.crypto.ff2048_dh(x, gy);
						default:
									throw new Error("only p256/ff2048 supported");
			}
};
...
/* 1.3 Client */
var set_onertt_hs_keys = function (hs, log) {
...
var set_onertt_data_keys = function (ms, log) {
...
var put_server_hello_13 = function (params, psk, x, log) {
			...
			if (checkServerHelloLog(log, params) === true) return { hsk: hsk, valid: true };else print_debug("provided server hello log is inconsistent with context");
			return { hsk: hsk, valid: false };
};

var put_server_finished_13 = function (params, cert, log1, sv, log2, sfk, vd, log3, ms) {
			...
			if (ProScript.crypto.verify_rsa_pss_sha256(pk, to_sign, sv) === true) {
						if (ProScript.crypto.hmac_sha256(sfk, log2) === vd) {
									if (checkServerCertificateLog(log1, params, cert) === true && checkServerCertificateVerifyLog(log2, params, cert) === true && checkServerFinishedLog(log3, params, cert) === true) return { keys: dk, valid: true };else print_debug("provided SC/SCV/SF logs are not consistent with context!");
						} else print_debug("server finished invalid!");
			} else print_debug("server certificate verify invalid!");
			return { keys: dk, valid: false };
};

var get_client_finished_no_auth_13 = function (params, cert, log3, cfk) {
			var cfin = ProScript.crypto.hmac_sha256(cfk, log3);
			if (checkServerFinishedLog(log3, params, cert) === true) return { cfin: cfin, valid: true };else print_debug("SF log inconsistent with context");
			return { cfin: cfin, valid: false };
};

var get_client_certificate_verify_13 = function (params, cert, certC, log1, sk) {
			var sg = ProScript.crypto.sign_rsa_pss_sha256(sk, sigval_client_13(log1));
			if (checkClientCertificateLog(log1, params, cert, certC) === true) return { sg: sg, valid: true };else print_debug("CC log inconsistent with context");
			return { sg: sg, valid: false };
};

var get_client_finished_client_auth_13 = function (params, cert, certC, log2, cfk) {
			var cfin = ProScript.crypto.hmac_sha256(cfk, log2);
			if (checkClientCertificateVerifyLog(log2, params, cert, certC) === true) return { cfin: cfin, valid: true };else print_debug("CCV log inconsistent with context");
			return { cfin: cfin, valid: false };
};
...
var put_server_hello_done_no_auth_12 = function (params, x, cert, dhp, sv, log) {
			...
			let ms = ProScript.crypto.tls12_prf_label(pms, 'master secret', params.ext_ms ? log : params.cr + params.sr, 48);

			let cvd = ProScript.crypto.tls12_prf_label(ms, 'client finished', log, 12);
			...
			if (ProScript.crypto.verify_rsa_sha256(pk, to_sign, sv) === true) {
						if (checkClientKeyExchangeLog12(log, params, cert) === true && checkServerKeyExchangeParams12(dhp, params.gy) === true) return { dk: dk, ms: ms, cfin: cvd, valid: true };else print_debug("provided SKE/CKE log is inconsistent with context");
...
var put_server_finished_no_auth_12 = function (params, cert, log, sfin, ms) {
			let svd = ProScript.crypto.tls12_prf_label(ms, 'server finished', log, 12);
			if (svd === sfin) {
						if (checkServerPreFinishedLog12(log, params, cert) === true) return true;else print_debug("provided SF log is inconsistent with context");
...
var put_client_hello_13 = function (cr, psk, gn, gx, log) {
			...
			if (checkClientHelloLog(log, cr, gx) === true) return { sr: sr, gy: gy, hs: hs, valid: true };else print_debug("CH log inconsistent with context");
...
var get_server_certificate_verify_13 = function (params, log1, hs, cert, log2, sk) {
			...
			if (checkServerHelloLog(log1, params) === true && checkServerCertificateLog(log2, params, cert) === true) return { hsk: hs_keys, sg: sg, valid: true };else print_debug("SH/SC log inconsistent with context");
...
var get_server_finished_13 = function (params, cert, log, sfk) {
			...
			if (checkServerCertificateVerifyLog(log, params, cert) === true) return { sfin: sfin, valid: true };else print_debug("CV log inconsistent with context");
			return { sfin: sfin, valid: false };
};

var sigval_server_13 = function (hash) {
			var sigval = '20202020202020202020202020202020202020' + '20202020202020202020202020202020202020' + '20202020202020202020202020202020202020' + '20202020202020' + util.a2hex('TLS 1.3, server CertificateVerify') + '00' + hash;

			return sigval;
};

var sigval_client_13 = function (hash) {
			var sigval = '20202020202020202020202020202020202020' + '20202020202020202020202020202020202020' + '20202020202020202020202020202020202020' + '20202020202020' + util.a2hex('TLS 1.3, client CertificateVerify') + '00' + hash;
			return sigval;
};

var put_client_finished_no_auth_13 = function (params, cert, log, cfk, cfin) {
			if (ProScript.crypto.hmac_sha256(cfk, log) === cfin) {
						if (checkServerFinishedLog(log, params, cert) === true) return true;else {
...
var put_client_finished_client_auth_13 = function (params, cert, certC, log1, cv, log2, cfk, cfin) {
			let pkC = ProScript.crypto.pkcert(certC);
			if (ProScript.crypto.verify_rsa_pss_sha256(pkC, sigval_client_13(log1), cv) === true && ProScript.crypto.hmac_sha256(cfk, log2) === cfin) {
						if (checkClientCertificateLog(log1, params, cert, certC) === true && checkClientCertificateVerifyLog(log2, params, cert, certC) === true) return true;else {
...
var get_server_hello_done_12 = function (cr, sr, gy, dhp, sk) {
			...
			if (checkServerKeyExchangeParams12(dhp, gy) === true) return { sg: sg, valid: true };else print_debug("provided SKE is not consistent with params");
			return { sg: sg, valid: false };
};

var put_client_ccs_no_auth_12 = function (params, y, cert, log1) {
			let pms = dh_compute_secret(params.gn, y, params.gx);
			let ms = ProScript.crypto.tls12_prf_label(pms, 'master secret', params.ext_ms ? log1 : params.cr + params.sr, 48);
			let dk = deriveKeys_12(params.ae, ms, params.sr + params.cr);
...
var put_client_finished_12 = function (params, cert, ms, log1, cfin, log2) {
			let cvd = ProScript.crypto.tls12_prf_label(ms, 'client finished', log1, 12);
			let svd = ProScript.crypto.tls12_prf_label(ms, 'server finished', log2, 12);
			if (cvd === cfin) {
						if (checkClientKeyExchangeLog12(log1, params, cert) === true && checkServerPreFinishedLog12(log2, params, cert) === true) return { cfin: cvd, sfin: svd, valid: true };else print_debug("provided CKE/SF log is inconsistent with context");
...
module.exports = {
			get_client_hello: function () {
						...
						ProScript.state.insert('clientEphemeralSecrets', sCH.cr, sCH.keys);
						return {
									cr: sCH.cr,
			...
			put_server_hello_13: function (cr, params, ticket, log) {
						var sec = ProScript.state.get('clientEphemeralSecrets', cr);
						...
						var rSH = put_server_hello_13(params, psk, sec[params.gn].dh_private, log);
						ProScript.state.remove('clientEphemeralSecrets', cr);
						if (rSH.valid) {
									ProScript.state.insert('clientHandshakeKeys', cr, { params: params,
												keys: rSH.hsk.hk,
												ms: rSH.hsk.ms,
												cfk: rSH.hsk.cfk,
												sfk: rSH.hsk.sfk });
			...
			put_server_finished_13: function (cr, cert, log1, sv, log2, sfin, log3) {
						var hs_keys = ProScript.state.get('clientHandshakeKeys', cr);

						var rSF = put_server_finished_13(hs_keys.params, cert, log1, sv, log2, hs_keys.sfk, sfin, log3, hs_keys.ms);
						ProScript.state.insert('clientMasterSecrets', cr, { params: hs_keys.params,
									cert: cert,
									cfk: hs_keys.cfk,
									sfk: hs_keys.sfk,
									ms: hs_keys.ms });
						ProScript.state.insert('clientDataKeys', cr, { params: hs_keys.params, keys: rSF.keys.tk });
						return;
			},

			get_client_finished_no_auth_13: function (cr, log) {
						var ms_sec = ProScript.state.get('clientMasterSecrets', cr);
						var rCF = get_client_finished_no_auth_13(ms_sec.params, ms_sec.cert, log, ms_sec.cfk);
						return rCF;
			},

			put_session_ticket_13: function (cr, log, tick) {
						var ms_sec = ProScript.state.get('clientMasterSecrets', cr);
			...
			put_client_hello_13: function (cr, gx, ticket, log) {
						...
						let sr = rCH.sr;
						let gy = rCH.gy;
						ProScript.state.insert('serverEphemeralSecrets', sr, { cr: cr, gx: gx, hs: rCH.hs });
						return {
									sr: sr,
									gy: gy,
									psk_ctx: psk_context
			...
			get_server_certificate_verify_13: function (sr, params, log1, cert, log2) {
						let hs_sec = ProScript.state.get('serverEphemeralSecrets', sr);
						...
						if (res.valid && params.cr === hs_sec.cr && params.gx === hs_sec.gx) {
									...
									ProScript.state.insert('serverHandshakeKeys', sr, { params: params, keys: peerKeys(res.hsk.hk) });
									ProScript.state.insert('serverMasterSecrets', sr, { params: params, cert: cert, ms: res.hsk.ms, cfk: res.hsk.cfk, sfk: res.hsk.sfk });
									return { sg: res.sg, valid: res.valid };
						} else return { sg: res.sg, valid: res.valid };
			},

			get_server_finished_13: function (sr, log) {
						var ms_sec = ProScript.state.get('serverMasterSecrets', sr);
						var sSF = get_server_finished_13(ms_sec.params, ms_sec.cert, log, ms_sec.sfk);
						return sSF;
			},

			put_client_finished_no_auth_13: function (sr, log, cfin, ticket) {
						var ms_sec = ProScript.state.get('serverMasterSecrets', sr);
						let dk = set_onertt_data_keys(ms_sec.ms, log);
						ProScript.state.insert('serverDataKeys', sr, { params: ms_sec.params, keys: peerKeys(dk.tk) });
						if (put_client_finished_no_auth_13(ms_sec.params, ms_sec.cert, log, ms_sec.cfk, cfin)) {
									var sSF = set_rms_ems(ms_sec.ms, log);
									ProScript.state.remove('serverMasterSecrets', sr);
									ProScript.state.insert('serverSessions', ticket, { params: ms_sec.params,
												cert: ms_sec.cert,
												ticket: ticket,
												rms: sSF.rms,
												ems: sSF.ems });
			...
			put_server_hello_done_no_auth_12: function (cr, params, cert, dhp, sv, log) {
						var sec = ProScript.state.get('clientEphemeralSecrets', cr);
						...
						//console.log(rSH);
						if (rSH.valid || debug) {
									ProScript.state.insert('clientDataKeys', cr, { params: params, keys: rSH.dk });
									//	console.log(KVS);
									ProScript.state.insert('clientMasterSecrets', cr, { params: params,
												cert: cert,
												cfk: rSH.ms,
												sfk: rSH.ms,
												ms: rSH.ms });
			...
			put_server_finished_no_auth_12: function (cr, tick, log, sfin) {
						let ms_sec = ProScript.state.get('clientMasterSecrets', cr);
						let res = put_server_finished_no_auth_12(ms_sec.params, ms_sec.cert, log, sfin, ms_sec.ms);
						if (res === true && tick !== '') ProScript.state.insert('clientSessions', tick, { params: ms_sec.params,
									cert: ms_sec.cert,
									ticket: tick,
									rms: ms_sec.ms,
									ems: ms_sec.ms });
			...
			put_client_hello_12: function (cr, cert) {
						let rCH = put_client_hello_12(cr, "p256");
						ProScript.state.insert('serverEphemeralSecrets12', rCH.sr, { cr: cr,
									cert: cert,
									gy: rCH.gy,
									y: rCH.y });
			...
			get_server_hello_done_12: function (sr, dhp) {
						let s_sec = ProScript.state.get('serverEphemeralSecrets12', sr);
						let sk = ProScript.crypto.server_key;
						let sg = get_server_hello_done_12(s_sec.cr, sr, s_sec.gy, dhp, sk);
						return sg;
			},

			put_client_ccs_no_auth_12: function (sr, params, log1) {

						let s_sec = ProScript.state.get('serverEphemeralSecrets12', sr);
						let ks = put_client_ccs_no_auth_12(params, s_sec.y, s_sec.cert, log1);
						if (ks.valid) {
									ProScript.state.insert('serverDataKeys', sr, { params: params, keys: peerKeys(ks.dk) });
									ProScript.state.insert('serverMasterSecrets', sr, { params: params, cert: s_sec.cert,
			...
			put_client_finished_12: function (sr, params, log1, cfin, ticket, log2) {
						var ms_sec = ProScript.state.get('serverMasterSecrets', sr);
			...
			encrypt_13: function (index, table, plain, ct) {
			...
			decrypt_13: function (index, table, cipher) {
			...
			encrypt_12: function (index, table, plain, ad) {
						print_debug("encrypting with " + table + " for " + index);
						//	console.log(KVS);
						var keys = ProScript.state.get(table, index).keys;
						print_debug("got keys!");
			...
			decrypt_12: function (index, table, cipher, ad) {
						print_debug("decrypting with " + table + " for " + index);
						var keys = ProScript.state.get(table, index).keys;
						print_debug("got keys!");
						print_debug(keys);
			...
			getClientSession: function (ticket) {
						let s = ProScript.state.get("clientSessions", ticket);
						if (s) return { params: s.params, cert: s.cert, ticket: s.ticket };else return undefined;
			},
			getServerSession: function (ticket) {
						let s = ProScript.state.get("serverSessions", ticket);
						if (s) return { params: s.params, cert: s.cert, ticket: s.ticket };else return undefined;
			}
};
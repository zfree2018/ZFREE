...
const defaultISPConfig = function () {
	return {
		ContentType: 1,
		Policy: 1,
		Address_List_Length: 32,
		Address_List: []
	};
};
...
const defaultCipherState = function () {
	return {
		role: "client",
...
const handshakeRecord = function (pv, frag) {
...
const negotiate = function (cipherState, ch, sr) {
	...
	if (ch_pv == formats.PV.TLS_1p3 && util.mem(formats.CS.TLS_AES_128_GCM_SHA256, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p3;
		cs = formats.CS.TLS_AES_128_GCM_SHA256;
		sh.protocol_version = "7f14"; //NOTE: Will become == pv in final RFC
		sh.cipher_suite = cs;
		if (ch.extensions[formats.EXT.key_share][0].dh_group == formats.SG.secp256r1) sh.extensions[formats.EXT.key_share] = {
			dh_group: formats.SG.secp256r1,
			dh_public: cipherState.params.gy
		};else if (ch.extensions[formats.EXT.key_share][0].dh_group == formats.SG.ffdhe2048) sh.extensions[formats.EXT.key_share] = {
		...
		if (ch.extensions[formats.EXT.pre_shared_key] != undefined) {
			...
			if (sess && sess.params.pv === formats.PV.TLS_1p3 && sess.params.cs === formats.CS.TLS_AES_128_GCM_SHA256) {
	...
	} else if (ch_pv == formats.PV.TLS_1p2 && util.mem(formats.CS.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p2;
		cs = formats.CS.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256;
		sh.protocol_version = pv;
		sh.cipher_suite = cs;
	} else if (ch_pv == formats.PV.TLS_1p2 && util.mem(formats.CS.TLS_DHE_RSA_WITH_AES_128_GCM_SHA256, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p2;
		cs = formats.CS.TLS_DHE_RSA_WITH_AES_128_GCM_SHA256;
		sh.protocol_version = pv;
		sh.cipher_suite = cs;
	} else if (ch_pv == formats.PV.TLS_1p2 && util.mem(formats.CS.TLS_RSA_WITH_AES_128_GCM_SHA256, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p2;
		cs = formats.CS.TLS_RSA_WITH_AES_128_GCM_SHA256;
		sh.protocol_version = pv;
		sh.cipher_suite = cs;
	} else if (util.mem(ch_pv, [formats.PV.TLS_1p0, formats.PV.TLS_1p1, formats.PV.TLS_1p2]) && util.mem(formats.CS.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p0;
		cs = formats.CS.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA;
		sh.protocol_version = pv;
		sh.cipher_suite = cs;
	} else if (util.mem(ch_pv, [formats.PV.TLS_1p0, formats.PV.TLS_1p1, formats.PV.TLS_1p2]) && util.mem(formats.CS.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p0;
		cs = formats.CS.TLS_DHE_RSA_WITH_AES_128_CBC_SHA;
		sh.protocol_version = pv;
		sh.cipher_suite = cs;
	} else if (util.mem(ch_pv, [formats.PV.TLS_1p0, formats.PV.TLS_1p1, formats.PV.TLS_1p2]) && util.mem(formats.CS.TLS_RSA_WITH_AES_128_CBC_SHA, ch.cipher_suites)) {
		pv = formats.PV.TLS_1p0;
...
const validateNegotiation = function (cipherState, sh) {
	...
	if (sh_pv !== formats.PV.TLS_1p3 && !util.mem(sh.compression, ch.compressions)) {
		throw new Error("server chose a compression method that was not in client hello");
	}

	if (ch.protocol_version == formats.PV.TLS_1p3 && ch.extensions[formats.EXT.pre_shared_key] != undefined && ch.extensions[formats.EXT.pre_shared_key][0].psk_identity === sh.extensions[formats.EXT.pre_shared_key].psk_identity) {
...
const getLogHash13 = function (log, psk_ctx) {
...
const getLogHash12 = function (log) {
	return tls_crypto.sha256(log);
};

const TLS_client_callbacks = {
	hs_send_client_hello: function (cipherState) {
		//let sCH = tls_core.get_client_hello(defaultISPConfig());
		let sCH = tls_core.get_client_hello();
		let ch = formats.defaultClientHello(cipherState.config, sCH.cr, sCH.public_values);
  ...
  */
		...
		cipherState.log += chb;
		let r = handshakeRecord(cipherState.params.pv, chb);
		return [r];
	},
  ...
  */
  ...
  if (cipherState.session.pv == formats.PV.TLS_1p3) {
	...
	hs_recv_server_hello: function (msgs, cipherState) {
		...
		if (msgs.length != 1 || msgs[0].ht !== formats.HT.server_hello) {
		...
		if (cipherState.params.pv === formats.PV.TLS_1p3) {
			...
			var rSH = tls_core.put_server_hello_13(cipherState.params.cr, cipherState.params, cipherState.ticket, getLogHash13(cipherState.log));
  ...
  */
	...
	hs_recv_server_hello_done: function (msgs, cipherState) {
		...
		switch (cipherState.params.kex) {
			case "ECDHE":
				if (msgs.length === 3 && cipherState.params.pv !== formats.PV.TLS_1p3 && msgs[0].ht === formats.HT.certificate && msgs[1].ht === formats.HT.server_key_exchange && msgs[1].pl.kex === cipherState.params.kex && msgs[1].pl.ec_params.curve === formats.SG.secp256r1 && msgs[2].ht === formats.HT.server_hello_done) {
					...
					let ckeb = formats.clientKeyExchangeBytes(formats.defaultClientKeyExchange_ECDHE(cipherState.params.gx), cipherState.params.pv);
					cipherState.log += ckeb;
					out_msgs.push(handshakeRecord(cipherState.params.pv, ckeb));
					let fin = tls_core.put_server_hello_done_no_auth_12(cipherState.params.cr, cipherState.params, msgs[0].pl.chain, dhp, msgs[1].pl.sig.sig_value, getLogHash12(cipherState.log));
					...
					throw new Error('only non-auth ecdhe 1.2 implemented 1: ' + tls_pp.printLog(msgs));
				}
			case "DHE":
				throw new Error('only ecdhe 1.2 implemented 2: ' + tls_pp.printLog(msgs));
			case "RSA":
				throw new Error('only ecdhe 1.2 implemented 3: ' + tls_pp.printLog(msgs));
			default:
				throw new Error('only ecdhe 1.2 implemented 4: ' + tls_pp.printLog(msgs));
		}
	},
	hs_recv_server_finished: function (msgs, cipherState) {
		var fin_msg;
		if (cipherState.params.pv === formats.PV.TLS_1p3 && msgs.length === 4 && msgs[0].ht === formats.HT.encrypted_extensions && msgs[1].ht === formats.HT.certificate && msgs[2].ht === formats.HT.certificate_verify && msgs[3].ht === formats.HT.finished) {
		...
		} else if (cipherState.params.pv !== formats.PV.TLS_1p3 && msgs.length === 1 && msgs[0].ht === formats.HT.finished) {
			...
			if (res) Sessions[cipherState.host] = cipherState.ticket;else throw new Error('server finished failed in tls 1.2');
	...
	app_data_send: function (msg, cipherState) {
		...
		return [{
			type: formats.ContentType.application_data,
			version: cipherState.params.pv,
			fragment: util.a2hex(`GET ${cipherState.path} HTTP/1.1`) + '0d0a' + util.a2hex('Host: ' + cipherState.host) + '0d0a0d0a'
		}];
	}
};
...
const getDHKey = function (keyshares, gn) {
	let i;
	for (i = 0; i < keyshares.length; i++) if (keyshares[i].dh_group === gn) return keyshares[i].dh_public;
	return undefined;
};

const TLS_server_callbacks = {
	hs_recv_client_hello: function (msgs, cipherState) {
		if (msgs.length === 1 && msgs[0].ht === formats.HT.client_hello) {
			cipherState.log = msgs[0].to_log;
			...
			if (cipherState.params.pv === formats.PV.TLS_1p3) {
				let p256key = getDHKey(ch.extensions[formats.EXT.key_share], formats.SG.secp256r1);
				let ff2048key = getDHKey(ch.extensions[formats.EXT.key_share], formats.SG.ffdhe2048);
				if (p256key) {
					var rCH = tls_core.put_client_hello_13(ch.client_random, p256key, cipherState.ticket, getLogHash13(cipherState.log));
				...
				if (cipherState.params.kex === "ECDHE") {
					var rCH = tls_core.put_client_hello_12(ch.client_random, tls_crypto.server_cert);
					sh.server_random = rCH.sr;
					cipherState.params.sr = rCH.sr;
					cipherState.params.cr = ch.client_random;
					cipherState.params.gn = "p256";
	...
	hs_send_server_hello_done: function (msgs, cipherState) {
		...
		if (cipherState.params.kex == 'ECDHE') {
			...
			return [handshakeRecord(cipherState.params.pv, cb), handshakeRecord(cipherState.params.pv, skeb), handshakeRecord(cipherState.params.pv, shdb)];
	...
	hs_send_server_finished: function (msgs, cipherState) {
		...
		cipherState.log += eeb;
		out_msgs.push(handshakeRecord(cipherState.params.pv, eeb));
		var sc = formats.defaultServerCertificate;
		let scb = formats.certificateBytes(sc, cipherState.params.pv);
		cipherState.log += scb;
		out_msgs.push(handshakeRecord(cipherState.params.pv, scb));
		...
		out_msgs.push(handshakeRecord(cipherState.params.pv, scvb));

		var hash3 = getLogHash13(cipherState.log);

		var sSF = tls_core.get_server_finished_13(cipherState.params.sr, hash3);
  ...
  */
	...
	hs_recv_client_ccs: function (msgs, cipherState) {
		if (msgs.length === 1 && msgs[0].ht === formats.HT.client_key_exchange) {
			var cke = msgs[0].pl;
			cipherState.log += msgs[0].to_log;
			...
			let res = tls_core.put_client_ccs_no_auth_12(cipherState.params.sr, cipherState.params, getLogHash12(cipherState.log));
  ...
  */
	...
	hs_recv_client_finished: function (msgs, cipherState) {
		if (cipherState.params.pv === formats.PV.TLS_1p3) {
			if (msgs.length === 1 && msgs[0].ht === formats.HT.finished) {
				var cfin = msgs[0].pl;
				let log1 = getLogHash13(cipherState.log);
				cipherState.log += msgs[0].to_log;
			...
			if (msgs.length === 1 && msgs[0].ht === formats.HT.finished) {
				...
				let res = tls_core.put_client_finished_12(cipherState.params.sr, cipherState.params, log1, cfin.verify_data, "", getLogHash12(cipherState.log));
	...
	app_data_recv: function (msg, cipherState) {
		...
		if (cipherState.params.pv == formats.PV.TLS_1p3) {
			var ticket = {
				ticket_lifetime: "00000000",
				ticket_age_add: "00000000",
